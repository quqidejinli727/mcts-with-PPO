...
def test_optimization_from_segment_assignments(
    ...
    params.json overrides:
        log_interval (int): log & density-check interval (default: 30)
        early_stop_enable (int): 0=off, 1=on
        early_stop_mode (int): 0=trough, -1=trough-1step, 1=trough+1step
        early_stop_density_patience (int): consecutive density increases to confirm rebound
    """
    logging.info("=" * 70)
    ...
    # Allow params.json to override log_interval (function default is fallback)
    if hasattr(params, "log_interval"):
    ...
    # Early stopping state (configured via params.json)
    early_stop_enabled = int(getattr(params, "early_stop_enable", 0)) == 1
    early_stop_mode = int(getattr(params, "early_stop_mode", 0))
    early_stop_density_patience = int(getattr(params, "early_stop_density_patience", 2))
    density_history: List[Tuple[int, float]] = []  # (iteration, density) at log checks
    early_stop_triggered = False
    early_stop_target_iter = -1  # used by mode 1: run until this iteration
    # checkpoint window must cover trough ± log_interval for any mode
    _max_checkpoints = max(early_stop_density_patience + 2, 3)
    _checkpoints: List[Dict] = []  # rolling checkpoints
    if early_stop_enabled:
        mode_names = {-1: f"trough-{log_interval}", 0: "trough", 1: f"trough+{log_interval}"}
        logging.info(
            "Early stopping: enabled (mode=%d=%s, density_patience=%d, "
            "log_interval=%d, checkpoint_window=%d)",
            early_stop_mode, mode_names.get(early_stop_mode, "?"),
            early_stop_density_patience, log_interval, _max_checkpoints,
        )
    ...
    for iteration in range(max_iterations):
        ...
        if is_record or is_log:
            ...
            if is_log:
                ...
                # --- Density-based early stopping ---
                if early_stop_enabled and not early_stop_triggered:
                    density_history.append((iteration + 1, density))
                    # Save checkpoint every log interval
                    _checkpoints.append({
                        "optimizer_param_data": [p.data.clone() for p in all_opt_params],
                        "density_w": density_w.clone(),
                        "iter": iteration + 1,
                    })
                    if len(_checkpoints) > _max_checkpoints:
                        _checkpoints.pop(0)

                    # Need patience+1 points to detect patience consecutive increases
                    if len(density_history) >= early_stop_density_patience + 1:
                        recent = density_history[-(early_stop_density_patience + 1):]
                        # Rebound = ALL last 'patience' intervals show increase
                        all_increasing = all(
                        ...
                        if all_increasing:
                            ...
                            # Trough = first point of the increasing sequence
                            #   (the density minimum before rebound)
                            trough_iter = recent[0][0]
                            trough_density = recent[0][1]
                            confirm_iter = recent[-1][0]

                            # Target iter for each mode (relative to trough)
                            if early_stop_mode == 0:
                                target_iter = trough_iter
                            elif early_stop_mode == -1:
                                target_iter = max(trough_iter - log_interval, log_interval)
                            elif early_stop_mode == 1:
                                target_iter = min(trough_iter + log_interval, max_iterations)
                            else:
                                target_iter = trough_iter

                            _mode_names = {-1: f"trough-{log_interval}", 0: "trough", 1: f"trough+{log_interval}"}
                            logging.info(
                                "Density rebound: trough at iter %d (density=%.4f), "
                                "confirmed at iter %d. "
                                "Mode=%d (%s) → target iter %d.",
                                trough_iter, trough_density, confirm_iter,
                                early_stop_mode,
                                _mode_names.get(early_stop_mode, "?"),
                                target_iter,
                            )

                            if target_iter <= iteration + 1:
                                # We're already at or past target → restore checkpoint
                                if target_iter < iteration + 1:
                                    rollback_cp = None
                                    for cp in reversed(_checkpoints):
                                        if cp["iter"] == target_iter:
                                            rollback_cp = cp
                                            break
                                    if rollback_cp is not None:
                                        for p, saved in zip(
                                            all_opt_params,
                                            rollback_cp["optimizer_param_data"],
                                        ):
                                            p.data.copy_(saved)
                                        density_w = rollback_cp["density_w"]
                                        place_obj.density_weight.data.copy_(density_w)
                                        clamp_positions()
                                        sync_reuse_positions()
                                        logging.info(
                                            "Restored checkpoint from iter %d",
                                            rollback_cp["iter"],
                                        )
                                    else:
                                        logging.warning(
                                            "No checkpoint for iter %d, "
                                            "keeping current state", target_iter,
                                        )
                                break
                            else:
                                # Need to run more → mode 1 case
                                early_stop_target_iter = target_iter
                                logging.info(
                                    "Continuing optimization until iter %d",
                                    early_stop_target_iter,
                                )

                # Mode 1: check if we've reached the target iteration
                if early_stop_triggered and early_stop_target_iter > 0:
                    if iteration + 1 >= early_stop_target_iter:
                        logging.info(
                            "Early stopping (mode=1): reached target iter %d",
                            iteration + 1,
                        )
                        break

    total_time = time.time() - t_loop_start
    actual_iters = iteration + 1  # loop broke before incrementing, so this is the actual count
    logging.info("Optimization finished in %.2f s  (%d iters, avg %.3f s/iter)",
                 total_time, actual_iters, total_time / max(actual_iters, 1))
